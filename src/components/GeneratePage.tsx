      ...
      const jobs = items.map((part, index) => {
        ...
        return {
          ...
          metadata: {
            ...
            style: String(part.visual_style || '').trim() || (selectedStyles.length > 0 ? selectedStyles.join(', ') : 'cinematic documentary'),
            mood: String(part.mood || '').trim() || mood,
            camera: String(part.camera_style || '').trim() || camera,
            characterName: String(part.character_name || '').trim() || undefined,
            characterAnchor: String(part.character_anchor || '').trim() || undefined,
            characterNegativePrompt: String(part.character_negative_prompt || '').trim() || undefined,
